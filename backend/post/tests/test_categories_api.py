...
class PrivateCategoriesTests(TestCase):
    """Test the authroized user categories API"""
    ...
    def test_retrieve_categories(self):
        ...
        Category.objects.create(user=self.user, name='Cafe')
        Category.objects.create(user=self.user, name='Fast Food')
    ...
    def test_categories_limited_to_user(self):
        """Test that categories are for the authenticated user"""
        user2 = get_user_model().objects.create_user(
            'other@example.com',
            'secret1'
        )
        Category.objects.create(user=user2, name='Chinese')
        category = Category.objects.create(user=self.user, name='Japanese')
